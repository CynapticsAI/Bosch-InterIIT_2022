...
def train(student, generator, nz=256, device='cpu', epoch=0, epoch_itrs=501, g_iter=1, d_iter=5,
    ...
    for i in range(epoch_itrs):
        ...
        if i % 50 == 0:
            torch.save(student, "checkpoint/BlackBox_P1_" + str(i) + str(epoch) + ".pt")
            torch.save(generator, "checkpoint/generator_P1" + str(i) + "_" + str(epoch) + ".pt")



device = "cuda"
nz = 256
student = get_vid_resnet(num_classes=400).to(device=device)
